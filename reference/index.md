---
title: Reference
description: Reference documentation for the Town Ruins platform, including architecture, API, database, and repository guides.
tags:
  - reference
  - architecture
  - api
  - database
  - repository
aliases:
  - Reference Docs
  - Technical Reference
---

<div class="tr-folder-hero">

# Reference

<p>Quick-reference documentation for the Town Ruins platform, including architecture, API, database, and repository guides.</p>

</div>

<div class="tr-folder-nav">

<a class="tr-folder-nav-item" href="./repository_guide">
<span class="tr-folder-icon">&#128218;</span>
Repository Guide
</a>

<a class="tr-folder-nav-item" href="./architecture/">
<span class="tr-folder-icon">&#127963;</span>
Architecture
</a>

<a class="tr-folder-nav-item" href="./api/">
<span class="tr-folder-icon">&#128279;</span>
API
</a>

<a class="tr-folder-nav-item" href="./database/">
<span class="tr-folder-icon">&#128451;</span>
Database
</a>

</div>

<div class="tr-dashboard-section" style="margin-top: 1.5rem;">
<h3>Purpose</h3>
<p>This folder contains quick-reference documentation for the Town Ruins platform. It provides condensed views of architecture, API, database, and repository information for day-to-day development and operations.</p>
</div>

<div class="tr-dashboard-section">
<h3>Related Areas</h3>
<ul>
<li><a href="../architecture/">Architecture</a></li>
<li><a href="../api/">API Reference</a></li>
<li><a href="../database/">Database Reference</a></li>
<li><a href="../developer/">Developer Dashboard</a></li>
</ul>
</div>

<div class="tr-dashboard-section">
<h3>Frequently Accessed Pages</h3>
<ul>
<li><a href="./repository_guide">Repository Guide</a></li>
<li><a href="./architecture/">Architecture Reference</a></li>
<li><a href="./api/">API Reference</a></li>
<li><a href="./database/">Database Reference</a></li>
</ul>
</div>
